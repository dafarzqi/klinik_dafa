                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary"> 
                                <a href="tmbh_pasien.php"><button class="btn btn-primary">TAMBAH DATA</button></a>
                            </h6>
                        </div>
                        <div class="card-body">
            <?php $id=$_GET['id'];
            include '../koneksi.php';
            $data1 = mysqli_query($koneksi,"select * from pasien,user where user.id_user=pasien.id_user and pasien.id_pasien=$id");
            while ($d=mysqli_fetch_array($data1)) { ?>
            <form action="aksi_edit_pasien.php" method="post">
                 <input class="form-control" type="text" name="id_user" value="<?php echo $d['id_user']; ?>" hidden>
                 <input class="form-control" type="text" name="id_pasien" value="<?php echo $d['id_pasien']; ?>" hidden>
            <label>Nama</label>
            <input class="form-control" type="text" name="nama" value="<?php echo $d['nama']; ?>">
            <label>Kelas BPJS</label>
            <select name="bpjs" class="form-control">
                <option value="Kelas 1">Kelas 1</option>
                <option value="Kelas 2">Kelas 2</option>
                <option value="Kelas 3">Kelas 3</option>
                <option value="Non BPJS">Umum</option>
            </select>
            <label>Alamat</label>
            <input class="form-control" type="text" name="alamat" value="<?php echo $d['alamat']; ?>">
            <label>Foto</label>
            <input class="form-control" type="file" name="foto"><br>
            <img src="../img/<?php echo $d['foto']; } ?>" width=50 />
